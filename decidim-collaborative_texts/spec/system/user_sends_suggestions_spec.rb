...
describe "User adds suggestions" do
  ...
  context "with no documents" do
    it "shows an empty page with a message" do
      ...
      expect(page).to have_text("There are no collaborative texts yet")
    end
  end
  ...
  context "with documents" do
    ...
    it "lists all the documents" do
      ...
      expect(page).to have_no_text(unpublished_document.title)
    end
    ...
    it "shows the document details" do
      ...
      within("aside") do
        ...
        expect(page).to have_no_text("Second paragraph")
      end
      expect(page).to have_text("Suggestions are not allowed at this time")
      within ".collaborative-texts-container" do
        ...
        within(".collaborative-texts-suggestions-box") do
          ...
          expect(page).to have_text("Replace: A new content")
        end
    ...
    context "when suggestions are enabled" do
      ...
      it "shows the login modal" do
        ...
        expect(page).to have_text("To suggest changes, select or double-click")
        ...
        expect(page).to have_no_text("A selection is active")
        expect(page).to have_button("Suggest changes", disabled: true)
        within(".collaborative-texts-editor-container") do
          expect(page).to have_text("First paragraph")
        end
        ...
        select_text("#ct-node-3")
        expect(page).to have_text("A selection is active")
        within(".collaborative-texts-editor-container") do
          expect(page).to have_text("Edited First paragraph")
        end
      ...
      context "and user is logged in" do
        ...
        it "allows to create suggestions" do
          ...
          expect(page).to have_text("First paragraph", count: 1)

          click_on "2 suggestions"
          expect(page).to have_text("First paragraph", count: 2)
          expect(page).to have_text("Edited First paragraph", count: 1)
          expect(page).to have_text("A new content", count: 1)
