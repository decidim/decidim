...
class FixGidLeakingInElectionTitles < ActiveRecord::Migration[7.0]
  ...
  def up
    Election.find_each do |election|
      next if election.title.blank?

      updated_title = fix_gid_in_translatable_attribute(election.title)
      next if updated_title == election.title

      election.update_column(:title, updated_title) # rubocop:disable Rails/SkipsModelValidations
    end
  end
