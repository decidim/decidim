...
module Decidim::Elections::Votes
  describe LastVoteForVoter do
    ...
    describe "when there are votes for the voter id and the election" do
      let!(:last_voter_election_vote) { create(:vote, election: election, voter_id: voter_id, created_at: 1.day.from_now) }
      let!(:voter_election_votes) { create_list(:vote, 3, election: election, voter_id: voter_id) }
      let!(:recent_other_voters_vote) { create(:vote, election: election, created_at: 3.days.from_now) }
      let!(:other_voters_votes) { create_list(:vote, 2, election: election) }
      let!(:recent_other_elections_vote) { create(:vote, election: election, created_at: 3.days.from_now) }
      let!(:other_elections_votes) { create_list(:vote, 2, voter_id: voter_id) }
