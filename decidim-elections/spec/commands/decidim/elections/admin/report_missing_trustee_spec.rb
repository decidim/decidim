...
describe Decidim::Elections::Admin::ReportMissingTrustee do
  ...
  let(:method_name) { :report_missing_trustee }
  ...
  context "when valid form" do
    ...
    it "creates an action" do
      ...
      expect(action).to be_report_missing_trustee
    end

    it "calls the bulletin board method with the correct params" do
      subject.call
      expect(bulletin_board).to have_received(method_name).with(election.id, trustee.slug)
    end
  end
