...
module Decidim
  module Votings
    module Admin
      describe CreateMonitoringCommitteeMember do
        ...
        context "when using an existing user" do
          it "creates the memeber" do
            subject.call

            expect(Decidim::Votings::MonitoringCommitteeMember.count).to eq 1
            expect(Decidim::Votings::MonitoringCommitteeMember.first.voting).to eq voting
          end

          it "traces the action", versioning: true do
            expect(Decidim.traceability)
              .to receive(:perform_action!)
              .with(:create, Decidim::Votings::MonitoringCommitteeMember, current_user, resource: hash_including(:title))
              .and_call_original
        ...
        context "when inviting a new user" do
          ...
          describe "when the email already exists" do
            ...
            it "doesn't create a new user" do
              ...
              monitoring_committee_members = Decidim::Votings::MonitoringCommitteeMember.where(user: existing_user)

              expect(monitoring_committee_members.count).to eq 1
            end
