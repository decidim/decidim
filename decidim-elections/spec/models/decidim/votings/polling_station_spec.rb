...
module Decidim
  module Votings
    describe PollingStation do
      subject(:polling_station) { build(:polling_station, voting: voting, polling_station_managers: managers, polling_station_president: president) }
      ...
      context "when a polling station president is assigned" do
        context "when the officer is from the same voting" do
          let(:president) { create(:polling_officer, voting: voting, user: create(:user, organization: organization)) }
        ...
        context "when the officer is from another voting" do
          ...
          let(:president) { create(:polling_officer, voting: other_voting, user: create(:user, organization: organization)) }
