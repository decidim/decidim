...
module Decidim
  module Votings
    describe PollingOfficer do
      subject(:polling_officer) { build(:polling_officer, user: user, voting: voting) }
      ...
      context "when the user is already present" do
        context "when in the same voting" do
          let!(:other_polling_officer) { create(:polling_officer, user: user, voting: voting) }
        ...
        context "when in another voting" do
          ...
          let!(:other_polling_officer) { create(:polling_officer, user: user, voting: other_voting) }
