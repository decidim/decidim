...
describe Decidim::Comments::CommentCreatedEvent do
  ...
  let(:notification_title) { "There is a new comment from <a href=\"/profiles/#{comment_author.nickname}\">#{comment_author.name}</a> in <a href=\"#{resource_path}?commentId=#{comment.id}#comment_#{comment.id}\">#{resource_title}</a>" }
