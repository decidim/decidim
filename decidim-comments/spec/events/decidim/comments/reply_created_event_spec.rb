...
describe Decidim::Comments::ReplyCreatedEvent do
  ...
  let(:email_subject) { "#{comment_author_name} has replied your comment in #{translated resource.title}" }
  ...
  let(:notification_title) { "<a href=\"/profiles/#{comment_author.nickname}\">#{comment_author_name} @#{comment_author.nickname}</a> has replied your comment in <a href=\"#{resource_path}?commentId=#{comment.id}#comment_#{comment.id}\">#{translated resource.title}</a>" }
