...
describe Decidim::Comments::ReplyCreatedEvent do
  ...
  let(:notification_title) { "<a href=\"/profiles/#{comment_author.nickname}\">#{comment_author_name}</a> has replied your comment in <a href=\"#{resource_path}?commentId=#{comment.id}#comment_#{comment.id}\">#{resource_title}</a>" }
