  ...
  module Comments
    describe CommentByFollowedUserGroupEvent do
      ...
      describe "email_subject" do
        it "is correct" do
          expect(subject.email_subject).to eq("There is a new comment by #{html_escaped(user_group.name)} in #{resource_title}")
        end
      end
      ...
      describe "email_intro" do
        it "is correct" do
          expect(subject.email_intro)
            .to eq("The group #{html_escaped(user_group.name)} has left a comment in #{resource_title}. You can read it in this page:")
        end
      end
      ...
      describe "email_outro" do
        it "is correct" do
          expect(subject.email_outro)
            .to include("You have received this notification because you are following #{html_escaped(user_group.name)}")
        end
      end
      ...
      describe "notification_title" do
        it "is correct" do
          expect(subject.notification_title)
            .to start_with("There is a new comment by <a href=\"#{user_group_path}\">#{html_escaped(user_group.name)} @#{user_group.nickname}</a> in")
          expect(subject.notification_title)
