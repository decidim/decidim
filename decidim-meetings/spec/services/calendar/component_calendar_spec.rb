...
module Decidim::Meetings::Calendar
  describe ComponentCalendar do
    ...
    let!(:meeting) { create :meeting, :published, title: title }
    let!(:component) { meeting.component }
    let!(:another_meeting) { create(:meeting, :published, component: component, title: Decidim::Faker::Localized.localized { generate(:title) }) }
    let!(:external_meeting) { create(:meeting, title: Decidim::Faker::Localized.localized { generate(:title) }) }
    let!(:unpublished_meeting) { create(:meeting, component: component, title: Decidim::Faker::Localized.localized { generate(:title) }) }
    ...
    describe "#filters" do
      ...
      context "when no meetings returned" do
        let!(:online_meeting) { create :meeting, :published, :not_official, :online, component: component, title: title }
      ...
      context "when having meetings returned" do
        let!(:online_meeting) { create :meeting, :published, :official, :online, component: component, title: Decidim::Faker::Localized.localized { generate(:title) } }
