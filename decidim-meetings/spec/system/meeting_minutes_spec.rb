...
describe "Meeting minutes", type: :system do
  ...
  context "when meeting closing is not visible" do
    let(:visible) { false }

    it "the closing ingformation is not visible" do
      visit_meeting

      expect(page).to have_no_content("MEETING MINUTES")
      expect(page).to have_no_content("RELATED INFORMATION")
      expect(page).to have_no_css("div.card.card--secondary.extra")
      expect(page).to have_no_content("CONTRIBUTIONS COUNT")
      expect(page).to have_no_content("ATTENDEES COUNT")
      expect(page).to have_no_content("ATTENDING ORGANIZATIONS")
      expect(page).to have_no_css("div.minutes-section")
    end
  end
  ...
  context "when meeting minutes is visible" do
    ...
    it "shows information related with closing action" do
      visit_meeting

      expect(page).to have_content("MEETING MINUTES")
      expect(page).to have_css("div.card.card--secondary.extra")
      ...
      expect(page).to have_css("div.minutes-section")
    end

    context "and audio and video data is missing" do
      it "hides the minutes section with related information" do
        meeting.update(
          video_url: nil,
          audio_url: nil
        )
