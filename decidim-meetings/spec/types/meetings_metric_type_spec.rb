...
module Decidim
  module Meetings
    describe MeetingsMetricType, type: :graphql do
      ...
      let(:model) { Decidim::Meetings::MeetingsMetricResolver.new(current_organization) }
      let(:today) { Time.zone.today }
      let!(:models) do
        (0..4).each do |count|
          create(:metric, day: (today - count.days), cumulative: (4 - count), quantity: 1, metric_type: "meetings", organization: current_organization)
        end
      end
      ...
      describe "count" do
        ...
        it "returns the Meeting's last day cumulative count" do
