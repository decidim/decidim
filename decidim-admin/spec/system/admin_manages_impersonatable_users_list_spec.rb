...
describe "Admin manages impersonatable users list" do
  ...
  describe "listing impersonatable users" do
    ...
    it "shows each user and its managed status" do
      ...
      expect(page).not_to have_selector("tr[data-user-id=\"#{admin.id}\"]", text: admin.name)
      expect(page).not_to have_selector("tr[data-user-id=\"#{deleted.id}\"]", text: deleted.name)
      expect(page).not_to have_selector("tr[data-user-id=\"#{blocked.id}\"]", text: blocked.name)
