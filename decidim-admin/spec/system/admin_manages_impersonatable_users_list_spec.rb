...
describe "Admin manages impersonatable users list" do
  ...
  describe "listing impersonatable users" do
    ...
    it "shows each user and its managed status" do
      ...
      expect(page).not_to have_css("tr[data-user-id=\"#{external_not_managed.id}\"]", text: not_managed.name)
      expect(page).not_to have_css("tr[data-user-id=\"#{another_admin.id}\"]", text: another_admin.name)
      expect(page).not_to have_css("tr[data-user-id=\"#{user_manager.id}\"]", text: user_manager.name)
