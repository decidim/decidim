...
module Decidim
  module Admin
    ...
    class CategoriesController < Decidim::Admin::ApplicationController
      helper_method :current_featurable
      ...
      def new
        ...
        @form = form(CategoryForm).from_params({}, current_featurable: current_featurable)
      end

      def create
        authorize! :create, Decidim::Category
        @form = form(CategoryForm).from_params(params, current_featurable: current_featurable)

        CreateCategory.call(@form, current_featurable) do
          on(:ok) do
            flash[:notice] = I18n.t("categories.create.success", scope: "decidim.admin")
            redirect_to categories_path(current_featurable)
          end
      ...
      def edit
        ...
        @form = form(CategoryForm).from_model(@category, current_featurable: current_featurable)
      end
      ...
      def destroy
        ...
        DestroyCategory.call(@category) do
          ...
          redirect_back(fallback_location: categories_path(current_featurable))
        end
      end
      ...
      def collection
        @collection ||= current_featurable.categories.includes(:subcategories)
      end
    end
  end
end
