...
describe("RevocationsController", () => {
  ...
  // The confirm messages come pre-composed from the server.
  const formHtml = (name, totalMessage, impersonatedMessage) => `
    <form data-controller="revocations" data-revocations="${name}" data-revocations-count-url-value="/admin/verifications/count">
      ...
      <label>
        <input type="radio" name="revocations[impersonated_only]" value="false" data-revocations-option="total" data-confirm-message="${totalMessage}" data-revocations-target="option" data-action="change->revocations#pick">
      </label>
      <label>
        <input type="radio" name="revocations[impersonated_only]" value="true" data-revocations-option="impersonated" data-confirm-message="${impersonatedMessage}" data-revocations-target="option" data-action="change->revocations#pick">
      </label>
      <div data-revocations-date data-revocations-target="dateContainer" data-action="change->revocations#refresh selectDate->revocations#refresh">
        <input id="revocations_${name}_before_date_input" name="revocations[before_date]" type="date" data-revocations-target="date">
  ...
  beforeEach(async () => {
    fetch.mockReset()
    document.body.innerHTML = formHtml("dummy", "Revoke 5 authorizations", "Revoke 2 impersonated") +
      formHtml("other", "Revoke 3 authorizations", "Revoke 1 impersonated")
  ...
  it("reveals the actions bar and writes the confirm message of the picked option", () => {
    pick("dummy", "total")
  ...
  it("asks the server for the message matching the picked option and date", async () => {
    fetch.mockResolvedValue({ ok: true, json: () => Promise.resolve({ count: 3, message: "Revoke 3 impersonated before 15/03/2022" }) })
  ...
  it("writes the zero-count message when no authorization matches the picked date", async () => {
    fetch.mockResolvedValue({ ok: true, json: () => Promise.resolve({ count: 0, message: "Revoke 0 authorizations before 01/01/2019" }) })
  ...
  it("discards responses the form has already moved on from", async () => {
    ...
    radio("dummy", "total").checked = true
    resolveFetch({ ok: true, json: () => Promise.resolve({ count: 9, message: "Revoke 9 impersonated before 15/03/2022" }) })
    await flushPromises()

    expect(submit("dummy").dataset.confirm).toEqual("Revoking cannot be undone")
  })

  it("holds the submission with the generic confirm while the message is being fetched", () => {
  ...
  it("falls back to the dateless message synchronously when the date is cleared", () => {
    fetch.mockReturnValue(new Promise(() => {}))
  ...
  it("keeps the generic confirm when the server rejects the filters", async () => {
    fetch.mockResolvedValue({ ok: false, status: 422, json: () => Promise.resolve({ error: "invalid" }) })
