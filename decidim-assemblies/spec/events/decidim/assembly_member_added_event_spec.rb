...
describe Decidim::AssemblyMemberAddedEvent do
  ...
  let(:members_page) { decidim_assemblies.assembly_members_url(participatory_space, host:) }
  ...
  context "when membership is published" do
    ...
    context "when assembly is restricted" do
      let(:resource) { create(:assembly, :restricted, title: generate_localized_title(:assembly_title)) }
