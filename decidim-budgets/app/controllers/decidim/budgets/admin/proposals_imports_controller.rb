      ...
      class ProposalsImportsController < Admin::ApplicationController
        ...
        def create
          ...
          Admin::ImportProposalsToBudgets.call(@form) do
            on(:ok) do |proposals|
              flash[:notice] = I18n.t("proposals_imports.create.success", scope: "decidim.budgets.admin", number: proposals.length)
              redirect_to EngineRouter.admin_proxy(current_component).root_path
            end

            on(:invalid) do
              flash.now[:alert] = I18n.t("proposals_imports.create.invalid", scope: "decidim.budgets.admin")
              render action: "new"
            end
          end
        end
