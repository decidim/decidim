  ...
  module Budgets
    module Admin
      ...
      class ProjectForm < Decidim::Form
        ...
        def proposals
          resource_manifest = Decidim.find_resource_manifest(:proposals)
          @proposals ||= resource_manifest.try(:resource_scope, current_component)
                         &.order(title: :asc)
                         &.map { |proposal| [resource_manifest.presenter.constantize.new(proposal).title, proposal.id] }
        end
