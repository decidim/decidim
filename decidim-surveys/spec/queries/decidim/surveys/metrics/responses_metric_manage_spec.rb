...
describe Decidim::Surveys::Metrics::ResponsesMetricManage do
  ...
  let!(:responses) { create_list(:response, 5, questionnaire:, created_at: day) }
  let!(:old_responses) { create_list(:response, 5, questionnaire:, created_at: day - 1.week) }
  ...
  context "when executing" do
    ...
    it "updates metric records" do
      create(:metric, metric_type: "survey_responses", day:, cumulative: 1, quantity: 1, organization:, category: nil, participatory_space:, related_object: survey)
      registry = generate_metric_registry
