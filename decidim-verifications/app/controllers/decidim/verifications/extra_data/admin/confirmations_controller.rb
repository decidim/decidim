        ...
        class ConfirmationsController < Decidim::Admin::ApplicationController
          ...
          def create
            ...
            ConfirmUserAuthorization.call(@pending_authorization, @form) do
              on(:ok) do
                flash[:notice] = t("confirmations.create.success", scope: "decidim.verifications.extra_data.admin")
                redirect_to pending_authorizations_path
              end

              on(:invalid) do
                flash.now[:alert] = t("confirmations.create.error", scope: "decidim.verifications.extra_data.admin")
                render action: :new
              end
            end
          end
