...
describe Decidim::Verifications::ManagedUserErrorEvent do
  ...
  let(:notification_title) { "The participant <a href=\"/profiles/#{resource.current_user.nickname}\">#{resource.current_user.name}</a> has tried to verify themselves with the data of another participant (#{resource.managed_user.name})." }
  let(:email_subject) { "Failed verification attempt against another participant" }
  let(:email_intro) { "The participant <a href=\"http://#{organization_host}/profiles/#{resource.current_user.nickname}\">#{resource.current_user.name}</a> has tried to verify themselves with the data of another participant (#{resource.managed_user.name})." }
