...
class FixGidLeakingInDebateTitles < ActiveRecord::Migration[7.0]
  ...
  def up
    Debate.find_each do |debate|
      next if debate.title.blank?

      updated_title = fix_gid_in_translatable_attribute(debate.title)
      next if updated_title == debate.title

      debate.update_column(:title, updated_title) # rubocop:disable Rails/SkipsModelValidations
    end
  end
