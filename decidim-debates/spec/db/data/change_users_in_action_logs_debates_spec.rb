...
describe ChangeUsersInActionLogsDebates do
  ...
  describe "#up" do
    ...
    let!(:debate4) { create(:debate, component:, author: admin) }
    ...
    it "updates properly the action logs" do
      expect(Decidim::ActionLog.where(resource: debate4).first.user).to eq(admin)
      expect(Decidim::ActionLog.where(resource: debate3).first.user).to eq(user)
      expect(Decidim::ActionLog.where(resource: debate2).first.user).to eq(user)
      expect(Decidim::ActionLog.where(resource: debate1).first.user).to eq(user)
      migrator.migrate(:up)
      expect(Decidim::ActionLog.where(resource: debate4).first.user).to eq(admin)
      expect(Decidim::ActionLog.where(resource: debate3).first.user).to eq(user)
      expect(Decidim::ActionLog.where(resource: debate2).first.user).to eq(group)
      expect(Decidim::ActionLog.where(resource: debate1).first.user).to eq(group)
    end
