...
Instead `static_page_topics` are iterated into a new cell here `decidim-core/app/cells/decidim/footer_topics_cell.rb` along with their associated pages to footer under the Help column. This change to be shown in the footer is now seeded to application automatically but can deactivated in form via admin along with any newly created topic.
