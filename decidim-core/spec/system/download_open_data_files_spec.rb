...
describe "Download Open Data files", download: true do
  ...
  it "shows the page with the download links" do
    ...
    expect(page).to have_text("Here, you will find data files that are regularly generated from various deliberative and governance processes within")
    expect(page).to have_text("Download results in CSV format")
    expect(page).to have_text("These files are in CSV (Comma-Separated Values) format, which is a widely-used file format")

    click_on("How to open and work with these files")
    expect(page).to have_text("1. Download the files")

    click_on("Detailed explanation of each file")
    expect(page).to have_text("Below is a description of each dataset, including its schema (structure) and the type of information it contains")
    expect(page).to have_text("The component that the result belongs to")
    ...
    expect(page).to have_text("Participatory spaces")
    ...
    I18n.t("decidim.open_data.help").each do |section|
      ...
      section.last.each do |dict|
        expect(page).to have_text(I18n.t("decidim.open_data.help.#{section.first}.#{dict.first}"))
      end
    end

    click_on("License")
    expect(page).to have_text("Open Database License")
  end
end
