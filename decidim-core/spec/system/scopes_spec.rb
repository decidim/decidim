  ...
  describe "scope picker page" do
    ...
    context "when a scope is required" do
      ...
      it "does not show other organization top scopes in content" do
        other_scopes.each do |scope|
          expect(page).not_to have_css("[data-scope-picker][data-content] li a", text: scope.name["en"])
        end
      end
      ...
      context "when has a current scope" do
        ...
        it "allows to choose it" do
          expect(page).not_to have_selector("[data-scope-picker].picker-footer .buttons a.button[disabled='true']")
        end
      end
    end
    ...
    context "when has a root scope" do
      ...
      it "does not show global scope in header" do
        expect(page).not_to have_css("[data-scope-picker].picker-header li a", text: "Global scope")
      end
      ...
      it "does not show root sibling scope in header" do
        expect(page).not_to have_css("[data-scope-picker].picker-header li a", text: scopes.last.name["en"])
      end
      ...
      context "when has a current scope" do
        ...
        it "does not shows any scope in content" do
          expect(page).not_to have_css("[data-scope-picker][data-content] li")
        end
      end
    end
  end
