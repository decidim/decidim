    ...
    describe "#variant_url" do
      ...
      shared_examples "development local storage protocol options" do
        ...
        it "returns a URL containing the port only" do
          expect(subject.variant_url(:testing)).to match(%r{^http://#{hostname}:#{default_port}/rails/active_storage/disk/[^/]+/avatar\.jpg$})
        end
        ...
        context "when force_ssl is enabled" do
          ...
          it "returns a URL containing the port and protocol" do
            expect(subject.variant_url(:testing)).to match(%r{^https://#{hostname}:#{default_port}/rails/active_storage/disk/[^/]+/avatar\.jpg$})
          end
          ...
          context "and the PORT environment variable is defined as 3001" do
            ...
            it "returns a URL containing the port and protocol" do
              expect(subject.variant_url(:testing)).to match(%r{^https://#{hostname}:3001/rails/active_storage/disk/[^/]+/avatar\.jpg$})
            end
          end
          ...
          context "and the PORT environment variable is defined as 443" do
            ...
            it "returns a URL containing the protocol only" do
              expect(subject.variant_url(:testing)).to match(%r{^https://#{hostname}/rails/active_storage/disk/[^/]+/avatar\.jpg$})
            end
          end
        end
      ...
      context "with production environment" do
        ...
        it "returns a default URL" do
          expect(subject.variant_url(:testing, options)).to match(%r{^http://#{hostname}/rails/active_storage/disk/[^/]+/avatar\.jpg$})
        end
        ...
        context "and the PORT environment variable is defined as 443" do
          ...
          it "returns a URL containing the protocol only" do
            expect(subject.variant_url(:testing, options)).to match(%r{^https://#{hostname}/rails/active_storage/disk/[^/]+/avatar\.jpg$})
          end
        end
        ...
        context "and the PORT environment variable is defined as 8080" do
          ...
          it "returns a URL containing the port" do
            expect(subject.variant_url(:testing, options)).to match(%r{^http://#{hostname}:8080/rails/active_storage/disk/[^/]+/avatar\.jpg$})
          end
        end
        ...
        context "and force_ssl is enabled" do
          ...
          context "and the PORT environment variable is defined as 8080" do
            ...
            it "returns a URL containing the port and protocol" do
              expect(subject.variant_url(:testing, options)).to match(%r{^https://#{hostname}:8080/rails/active_storage/disk/[^/]+/avatar\.jpg$})
            end
          end
        end
      end
      ...
      context "with CDN host defined in the secrets" do
        ...
        it "returns a URL containing the CDN configurations" do
          expect(subject.variant_url(:testing)).to match(%r{^#{cdn_host}/rails/active_storage/blobs/redirect/[^/]+/avatar\.jpg$})
        end
      end
    end
  end
