  ...
  module Headers
    describe "ContentSecurityPolicy", type: :controller do
      ...
      it "sets the appropiate headers" do
        ...
        expect(response.headers["Content-Security-Policy"]).to include("img-src 'self' *.hereapi.com https://*.openstreetmap.org data: https://via.placeholder.com;")
      ...
      context "when content policy is added by organization" do
        ...
        it "sets the appropiate headers" do
          ...
          expect(response.headers["Content-Security-Policy"]).to include("img-src 'self' *.hereapi.com https://*.openstreetmap.org data: https://via.placeholder.com https://img.example.org;")
      ...
      context "when content policy is added via decidim config" do
        it "sets the appropiate headers" do
          ...
          expect(response.headers["Content-Security-Policy"]).to include("script-src 'self' 'unsafe-inline' 'unsafe-eval' https://script.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("style-src 'self' 'unsafe-inline' https://style.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("img-src 'self' *.hereapi.com https://*.openstreetmap.org data: https://img.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("connect-src 'self' *.hereapi.com *.jsdelivr.net https://connect.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("font-src 'self' https://font.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("frame-src 'self' https://frame.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("media-src 'self' https://example.org")
      ...
      context "when the organization has a content security policy" do
        ...
        it "sets the appropiate headers" do
          ...
          expect(response.headers["Content-Security-Policy"]).to include("script-src 'self' 'unsafe-inline' 'unsafe-eval' https://script.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("style-src 'self' 'unsafe-inline' https://style.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("img-src 'self' *.hereapi.com https://*.openstreetmap.org data: https://img.example.org https://via.placeholder.com;")
          expect(response.headers["Content-Security-Policy"]).to include("connect-src 'self' *.hereapi.com *.jsdelivr.net https://connect.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("font-src 'self' https://font.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("frame-src 'self' https://frame.example.org;")
          expect(response.headers["Content-Security-Policy"]).to include("media-src 'self' https://example.org")
