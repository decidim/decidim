...
describe Decidim::MetricCount do
  ...
  context "when executing a count request" do
    it "returns the metric cumulative count" do
      result = described_class.for(organization, metric)
      expect(result).to eq(5)
    end

    it "returns nothing for non present metric" do
      result = described_class.for(organization, "this-metric-does-not-exists")
  ...
  context "when executing a metric request" do
    it "returns the metric data" do
      result = described_class.for(organization, metric, counter_type: :metric)
      expect(result.size).to eq(1)
      expect(result.first).to eq([Time.zone.today, 5])
    end

    it "returns nothing for non present metric" do
      result = described_class.for(organization, "this-metric-does-not-exists", counter_type: :metric)
