  ...
  module Core
    describe SessionType do
      ...
      describe "verifiedUserGroups" do
        ...
        let!(:creator_user_group) { create(:user_group, :confirmed, organization:, users: []) }
        let!(:admin_user_group) { create(:user_group, :confirmed, organization:, users: []) }
        let!(:member_user_group) { create(:user_group, :confirmed, organization:, users: []) }
        let!(:verified_creator_user_group) { create(:user_group, :confirmed, :verified, organization:, users: []) }
        let!(:verified_admin_user_group) { create(:user_group, :confirmed, :verified, organization:, users: []) }
        let!(:verified_member_user_group) { create(:user_group, :confirmed, :verified, organization:, users: []) }
        let!(:requested_user_group) { create(:user_group, :confirmed, organization:, users: []) }
