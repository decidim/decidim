...
module Decidim
  describe Area do
    subject(:area) { build(:area, area_type: area_type, organization: organization) }
    ...
    context "with two areas with the same name, type and organization" do
      context "when no area_type is present" do
        let!(:existing_area) { create(:area, name: area.name, area_type: area_type, organization: organization) }
      ...
      context "when no area_type present" do
        let(:area_type) { nil }
        let!(:existing_area) { create(:area, name: area.name, organization: organization) }
