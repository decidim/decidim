        ...
        (Date.parse(day)..Date.current).each do |d|
          ...
          Decidim::Organization.find_each do |organization|
            if metric
              ...
              log_info "[#{organization.org_translated_name[organization.default_locale.to_s]}]: rebuilding metric [#{metric}] for day [#{current_day}]"
              call_metric_job(m_manifest, organization, current_day)
            else
              log_info "[#{organization.org_translated_name[organization.default_locale.to_s]}]: rebuilding all metrics for day [#{current_day}]"
              Decidim.metrics_registry.all.each do |metric_manifest|
