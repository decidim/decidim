...
shared_examples "visit unpublished resource with a share token" do
  ...
  context "when a share_token is provided" do
    let(:share_token) { create(:share_token, token_for: resource, token: SecureRandom.hex(32)) }
    ...
    context "when an invalid share_token is provided" do
      let(:share_token) { create(:share_token, :expired, token_for: resource, token: SecureRandom.hex(32)) }
    ...
    context "when the token requires the user to be registered" do
      let(:share_token) { create(:share_token, token_for: resource, registered_only: true, token: SecureRandom.hex(32)) }
