...
shared_examples "generates routes without query strings on slug" do
  ...
  context "with a component resource" do
    ...
    describe "#show" do
      ...
      it { is_expected.to eq("/#{admin_route_fragment}") }
    end

    describe "#edit" do
      subject { described_class.new(participatory_space).edit }

      it { is_expected.to eq("/#{admin_route_fragment}/edit") }
    end
  end
  ...
  context "with a polymorphic resource" do
    ...
    describe "#admin_index" do
      ...
      it { is_expected.to eq("/#{admin_route_fragment}/components/1/manage/dummy_resources/1/nested_dummy_resources") }
    end

    describe "#show" do
      subject { described_class.new([resource, nested_resource]).show }

      it { is_expected.to eq("/#{admin_route_fragment}/components/1/manage/dummy_resources/1/nested_dummy_resources/1") }
    end

    describe "#edit" do
      subject { described_class.new([resource, nested_resource]).edit }

      it { is_expected.to eq("/#{admin_route_fragment}/components/1/manage/dummy_resources/1/nested_dummy_resources/1/edit") }
    end
  end
