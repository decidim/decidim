...
shared_examples_for "reportable" do
  ...
  let(:moderation) { create(:moderation, reportable: subject, participatory_process: subject.feature.participatory_process, report_count: 1) }
  ...
  context "#hidden?" do
    ...
    context "when the resource has been hidden" do
      let(:moderation) { create(:moderation, reportable: subject, participatory_process: subject.feature.participatory_process, report_count: 1, hidden_at: Time.current) }
  ...
  context "#reported?" do
    context "when the report count is equal to 0" do
      let(:moderation) { create(:moderation, reportable: subject, participatory_process: subject.feature.participatory_process, report_count: 0) }
      let!(:report) { nil }
