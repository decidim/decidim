...
shared_examples "Endorse resource system specs" do
  ...
  context "when endorsements are enabled but blocked" do
    ...
    let(:disabled_button_selector) { Decidim.redesign_active ? ".proposal__actions button[disabled='true']" : ".proposal__actions span[disabled]" }
