...
module Decidim
  ...
  class FormBuilder < FoundationRailsHelper::FormBuilder
    ...
    def upload(attribute, options = {})
      ...
      template += content_tag(:div, class: "help-text") do
        ...
        if file.is_a?(Decidim::ImageUploader)
          help_scope = "decidim.forms.file_help.image"
        else
          help_scope = "decidim.forms.file_help.attachment"
        end
        help_messages = ["message_1", "message_2"]
        ...
        inner + content_tag(:ul) do
          ...
          if max_file_size
            file_size_mb = ((((max_file_size/1024/1024) * 100) / 100).round).inspect
            messages << I18n.t(
              "max_file_size",
