...
module Decidim
  class MachineTranslationUpdatedResourceJob < ApplicationJob
    ...
    def perform(resource, previous_changes, source_locale)
      ...
      translatable_fields.each do |field|
        ...
        locales.each do |locale|
          ...
          MachineTranslationUpdateFieldsJob.perform_later(id, class_name, field, resource_field(previous_changes, field, source_locale), locale, source_locale)
        end
      end
    end
    ...
    def resource_field(previous_changes, field, source_locale)
      values = previous_changes[field]
      new_value = values.last
