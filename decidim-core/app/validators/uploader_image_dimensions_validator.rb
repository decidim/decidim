...
class UploaderImageDimensionsValidator < ActiveModel::Validations::FileContentTypeValidator
  ...
  def validate_image_size(record, attribute, file, uploader)
    ...
    resolution_too_large = begin
      image.dimensions.any? { |dimension| dimension > uploader.max_image_height_or_width }
    rescue MiniMagick::Error
      true
    end
    record.errors.add attribute, I18n.t("carrierwave.errors.file_resolution_too_large") if resolution_too_large
  end
