...
By default, Decidim creates a context that is being passed to the form, containing 4 variables: `current_user`, `current_organization`, `current_participatory_process` and `current_component`.
...
Sometimes, you may need add the same parameter to all the forms in your controller, or you may need to override decidim default. You can achive this by using a concern as follows:
[source,ruby]
----
# frozen_string_literal: true
module MyOverride
...
Decidim::MyGem::MyForm.include(MyOverride)
----
