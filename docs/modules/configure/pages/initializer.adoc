...
This is useful for adding custom CSPs for external services like Here Maps, YouTube, Twitter etc.

Below you will find a the recepy of enabling Twitter timelines, based on https://content-security-policy.com/examples/twitter/[Content Security Policy (CSP) Examples Twitter example].
...
Additionally, the following example, will allow you to enable Jitsi meetings:

[source,ruby]
....
  {
    "connect-src": %w(meet.jit.si),
    "frame-src": %w(meet.jit.si),
  }
....
