...
As there are lots of dependencies in the whole database, currently an Organization cannot be deleted through the system panel. If you still want to go to this direction, there is a xref:develop:delete_organization.adoc[snippet for deleting an Organization with a script].
