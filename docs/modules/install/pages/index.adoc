...
have checked the `email_on_notification` option in their user panel. Also they can set the frequency they want
...
And this one, a weekly sumary. It takes the range between the time the task has been triggered and 7 days ago. It should be run in a weekly basis in cron (this is by setting the last prameter of the
cron format, which referes to the day of week).
----
