...
In this guide, we'll see how to install rbenv, PostgreSQL, Node.js and, Decidim, and how to configure everything together for a (and this is important) a development environment.
...
Using Rbenv-vars, you only need to perform one step (once) to add the automated ENV vars functionality to rbenv:
...
Then, you just need to edit, in any folder above your decidim copy, a file named `.rbenv-vars` and put your variables there:
...
=== 5.1 Alternatives with helper gems

But other approaches also works (or can be used for different environments, for instance use the .rbenv-var approach for developing and the figaro method for production).

For a working setup with figaro, you'll need to install it (but you don't need to do this if you are using rbenv-vars):
...
Be sure, to add `config/application.yml` to the `.gitignore` file!!!
