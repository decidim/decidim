...
For example, if the latest release is 0.29 we could decide to update to 0.30 or even 0.31.

To update, usually we recommend using small increments, as from now and then, we are changing Ruby version, or we perform upgrades to our dependencies, work that could break your application if you do major leaps. Therefore, if you need to upgrade from 0.29 to 0.31, we recommend to first upgrade to 0.30, follow the release notes, then, from 0.30 to upgrade to 0.31.

So, to perform the upgrade, we first follow the https://github.com/decidim/decidim/releases/tag/v0.30.0[release notes for 0.30], then, after we confirmed everything went ok, we should follow the https://github.com/decidim/decidim/releases/tag/v0.31.0[release notes for 0.31].
...
. Never add 2 or more versions of Decidim to the same release, as the database structure could change, deprecated functions could be removed, causing issues that may prevent you to successfully run post release commands.
