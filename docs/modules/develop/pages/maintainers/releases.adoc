...
Before you begin the release process, make sure you check if there are any open or pending  backports. Is not mandatory to open or merge all the backports, required, but is worth having an informed decision. Please refer to xref:develop:backports.adoc[backports] page.
...
. During the validation period, bugfixes must be implemented directly to the current `develop` branch, and if you need to backport, just add the red label `release: v0.xx` to allow automated backports.
 ... In special cases only (when the feature aimed to be fixed does not exist anymore in `develop`), you can open the Pull request directly against the release branch.
