...
. Update the version in the documentation system that we use, Antora. As its multi repository you'll need to do it on both places:
.. in the https://github.com/decidim/decidim[decidim repository]. This is done in https://github.com/decidim/documentation/blob/master/antora-playbook.yml[docs/antora.yml], by changing the version key, from `version: develop` to `version: v0.XX` (for instance `version: v0.27`).
Commit and push that change: `git add docs/antora.yml && git commit -m "Bump version for Antora" && git push origin release/x.y-stable`
.. in the https://github.com/decidim/documentation[documentation repository]:
