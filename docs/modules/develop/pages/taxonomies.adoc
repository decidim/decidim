...
- `taxonomies`: A JSON object with the tree structure of the taxonomies to be created. The keys are the names of the taxonomies (using the default locale for the organization) and the values are JSON objects with the properties along with the possible children taxonomies. The properties are:
