...
1. Configure postgresql with a super-user and save the database configuration in your shell environment. For instance, for a user called `decidim_development_app_user`
with password `changeme` and using the bash shell, it'd be:

[source,console]
----
sudo -u postgres psql -c "CREATE USER decidim_development_app_user WITH SUPERUSER CREATEDB NOCREATEROLE PASSWORD 'changeme'"
echo "export DATABASE_USERNAME=decidim_development_app_user" >> ~/.bashrc
echo "export DATABASE_PASSWORD=changeme" >> ~/.bashrc
source ~/.bashrc
----
