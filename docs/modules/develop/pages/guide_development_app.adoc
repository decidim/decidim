...
For better following this guide and as you should be working in a development environment (ie without sensitive data in your database),
for more comfort you can configure PostgreSQL with a relaxed security. This is not recommended for staging nor production configuration,
just for local development. These instructions are tested in Ubuntu.

[source,console]
----
export DATABASE_HOST=/var/run/postgresql/ >> ~/.bashrc
