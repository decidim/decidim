= Migrate to Shakapacker an instance app

In order to migrate an existing Decidim app to Shakapacker, there are a few changes your need to run in your code.
...
Shakapacker is a continuation of Webpacker performed by Shakacode.
...
=== Add Shakapacker to the application
...
* Install Decidim shakapacker custom code

[source,console]
----
bundle exec rails decidim:shakapacker:install
----

This task do a few steps to allow the Rails instance to have a shakapacker instance sharing the code between itself and Decidim gem.
...
`shakapacker:install` task should have created to you the following dirs structure:
...
Decidim Shakapacker provides a new configuration convention that allows you to add extra configurations for shakapacker using a configuration file named `config/assets.rb`. Within this file, you have the following API methods available:
...
Decidim::Shakapacker.register_stylesheet_import("stylesheets/your_app_extensions")

# If you want to do the same but include the SCSS file for the admin panel's
# main SCSS file, you can use the following method.
Decidim::Shakapacker.register_stylesheet_import("stylesheets/your_app_admin_extensions", group: :admin)
----
...
----
namespace :deploy do
  desc "Decidim shakapacker configuration"
  task :decidim_shakapacker_install do
    on roles(:all) do
  ...
  before "deploy:assets:precompile", "deploy:decidim_shakapacker_install"
end
----
...
If you have the following exception when executing `bundle exec rails decidim:upgrade` or `bundle exec rails decidim:shakapacker:install`
