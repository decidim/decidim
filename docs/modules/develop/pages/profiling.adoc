...
Bullet detects N+1 queries and suggests how to fix them, although it does not catch them all. It's currently configured in `config/initializers/bullet.rb` to log in the regular rails log and also in its own `log/bullet.log`. You will now see entries like the following:
