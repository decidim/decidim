...
Take into account that generating a new development application **overwrites** Webpacker configuration so these changes might be overwriten. That is why it's necessary, once the changes are stable enough, to update the generators files.
