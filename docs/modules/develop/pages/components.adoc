...
Take into account that generating a new development application **overwrites** Webpacker configuration so these changes might be overwriten. That's why it is necessary, once the changes are stable enough, to update the generators files.
