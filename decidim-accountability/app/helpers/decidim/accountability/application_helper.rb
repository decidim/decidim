  ...
  module Accountability
    ...
    module ApplicationHelper
      ...
      def categories_label
        translated(component_settings, :categories_label).presence || t("results.home.categories_label", scope: "decidim.accountability")
      end

      def subcategories_label
        translated(component_settings, :subcategories_label).presence || t("results.home.subcategories_label", scope: "decidim.accountability")
      end

      def heading_parent_level_results(count)
        text = translated(component_settings, :heading_parent_level_results).presence
        if text
      ...
      def heading_leaf_level_results(count)
        text = translated(component_settings, :heading_leaf_level_results).presence
        if text
          pluralize(count, text)
        else
          t("results.count.results_count", scope: "decidim.accountability", count: count)
