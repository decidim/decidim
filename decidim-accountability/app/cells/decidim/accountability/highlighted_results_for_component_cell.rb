    ...
    class HighlightedResultsForComponentCell < Decidim::ViewModel
      ...
      def results
        @results ||= case options[:order]
                     when "recent"
                       Decidim::Accountability::Result.where(component: model).includes(:taxonomies).order_by_most_recent
                     else
                       Decidim::Accountability::Result.where(component: model).includes(:taxonomies).order_randomly(random_seed)
                     end
      end
