...
module Decidim
  module Proposals
    describe UserGroupsThatCanEndorse do
      ...
      describe "when user has no user_groups" do
        it { expect { UserGroupsThatCanEndorse.from(current_user, proposal).to be_empty } }
      end
      ...
      describe "when user belongs to user_groups" do
        ...
        context "when user's user_groups are not verified" do
          it "ignores them" do
            ...
            expect { UserGroupsThatCanEndorse.from(current_user, proposal).to be_empty }
          end
        end
        ...
        context "when half of user user_groups are endorsed" do
          it "returns the other half of user's user_groups" do
            ...
            expected_endorsable_groups = [user_groups.last.id]
            expect(UserGroupsThatCanEndorse.from(current_user, proposal)).to eq expected_endorsable_groups
          end
        end
        context "when none of user's user_groups are endorsed" do
          it "offers all user_groups to endorse" do
            expected_endorsable_groups = user_groups.collect(&:id)
            expect(UserGroupsThatCanEndorse.from(current_user, proposal)).to eq expected_endorsable_groups
          end
        end
      end
