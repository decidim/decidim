...
module Decidim
  module Proposals
    describe AcceptAccessToCollaborativeDraft do
      ...
      let(:form) { AcceptAccessToCollaborativeDraftForm.from_params(form_params).with_context(current_user: current_user, current_organization: current_organization) }
      let(:form_params) do
        {
          state: state,
          id: id,
