...
describe Decidim::Proposals::Admin::UpdateProposalCategoryEvent do
  ...
  let(:email_intro) { "An admin has updated the category of your proposal \"#{decidim_escape_translated(resource.title)}\", check it out in this page:" }
  let(:email_outro) { "You have received this notification because you are the author of the proposal." }
  let(:notification_title) { "The <a href=\"#{resource_path}\">#{decidim_escape_translated(resource.title)}</a> proposal category has been updated by an admin." }
