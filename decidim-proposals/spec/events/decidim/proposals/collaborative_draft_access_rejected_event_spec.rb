...
describe Decidim::Proposals::CollaborativeDraftAccessRejectedEvent do
  ...
  context "when the notification is for coauthor users" do
    ...
    let(:notification_title) { %(<a href="#{requester_path}">#{requester_name}</a> has been <strong>rejected to access as a contributor</strong> of the <a href="#{resource_path}">#{resource_title}</a> collaborative draft.) }
