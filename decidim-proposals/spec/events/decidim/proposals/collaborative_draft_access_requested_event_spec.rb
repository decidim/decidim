...
describe Decidim::Proposals::CollaborativeDraftAccessRequestedEvent do
  ...
  context "when the notification is for coauthor users" do
    let(:notification_title) { %(<a href="#{requester_path}">#{requester_name}</a> requested access to contribute to the <a href="#{resource_path}">#{decidim_html_escape(resource_title)}</a> collaborative draft. Please <strong>accept or reject the request</strong>.) }
