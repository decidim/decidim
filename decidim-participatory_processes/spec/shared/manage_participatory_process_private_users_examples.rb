...
shared_examples "manage participatory process private users examples" do
  ...
  describe "when import a batch of private users from csv" do
    it "import a batch of participatory space private users" do
      ...
      expect(Decidim::Admin::ImportParticipatorySpacePrivateUserCsvJob).to receive(:perform_later).once.ordered.with("john.doe@example.org", "John Doe", participatory_process)
      expect(Decidim::Admin::ImportParticipatorySpacePrivateUserCsvJob).to receive(:perform_later).once.ordered.with("jane.doe@example.org", "Jane Doe", participatory_process)
