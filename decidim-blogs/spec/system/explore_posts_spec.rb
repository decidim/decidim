  ...
  context "when there are posts" do
    ...
    describe "index" do
      it "shows all posts for the given process" do
        ...
        expect(page).to have_css(".card--post", text: translated(new_post.title))
        expect(page).to have_css(".card--post", text: translated(old_post.title))
      end

      it "shows comment counts" do
        visit_component
        expect(page).to have_css('a[title="comments"]', text: "comment".pluralize(new_post.comments.count))
        expect(page).to have_css('a[title="comments"]', text: "comment".pluralize(old_post.comments.count))
      end

      it "shows endorsement counts" do
        visit_component
        expect(page).to have_css('a[title="endorsements"]', text: "endorsement".pluralize(new_post.endorsements.count))
        expect(page).to have_css('a[title="endorsements"]', text: "endorsement".pluralize(old_post.endorsements.count))
      end

      it "shows images" do
        visit_component
        expect(page).to have_css(".card--post img.card__image")
      end
